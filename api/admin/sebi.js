/**
 * api/admin/sebi.js — Vercel serverless function (Node)
 *
 * Phase 3 of the server-side DB access migration. Replaces the direct-Neon
 * admin SEBI-approval queries previously run from the SebiApprovals screen in
 * src/App.jsx. Admin-only: the caller's identity is derived from their
 * verified Firebase ID token, then their user_profiles.is_admin flag is
 * looked up server-side (never trusted from the client) — non-admins get 403.
 *
...
import { sql, setCors, requireAdmin, parseBody, sendAuthError } from '../_lib/auth.js';
