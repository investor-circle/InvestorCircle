  ...
  try {
    if (req.method === 'GET') {
      ...
      if (action === 'feed-config') {
        let uid;
        try { uid = await requireUid(req); } catch (e) { sendAuthError(res, e); return; }
        // `sql` times itself (see api/_lib/auth.js) — no manual wrap needed.
        // This was originally the CONTROL for the latency investigation: two
        // trivial indexed reads of small tables. If this reports a large db
        // figure, the cost is the database connection/wake, not the query.
      ...
      if (action === 'public-feed') {
        ...
        try { uid = await requireUid(req); } catch (e) { sendAuthError(res, e); return; }
        // `sql` times itself (see api/_lib/auth.js) — no manual wrap needed.
        // This is the heavy counterpart to feed-config's control reading:
        // same connection, much bigger query. The gap between the two db
        // figures is what this query actually costs; whatever they share is
