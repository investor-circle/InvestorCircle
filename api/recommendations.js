/**
 * api/recommendations.js — Vercel serverless function (Node)
 *
 * Phase 3 of the server-side DB access migration. Replaces the direct-Neon
 * recommendation queries previously run from src/db.js (createRecommendation,
 * getMyReceivedRecos, getMyMadeRecos, updateDelivery, setExitSignal,
 * cancelExitSignal, forwardRecommendation, deleteRecommendation,
 * deleteDelivery).
 *
 * GET  /api/recommendations?scope=received|made
 ...
 *
 * The caller's identity (myId/userId) always comes from the verified token.
 * All row-level mutations are scoped with WHERE clauses tying the row to
 * myId, matching the original src/db.js behavior exactly.
 */
