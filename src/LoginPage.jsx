...
export default function LoginPage() {
  ...
  const handleSignup = async () => {
    ...
    try {
      ...
      // Set displayName so AuthContext picks it up
      await updateProfile(cred.user, { displayName: fullName });

      // Upsert into Neon user_profiles with correct name fields
      if (sql) {
        try {
          await sql`
            ...
            ON CONFLICT (id) DO UPDATE SET
              ...
              updated_at = now()
          `;
        } catch (_) { /* AuthContext will upsert on auth state change — non-fatal */ }
      }
      // Auth state change fires → AuthContext logs user in → App.jsx pending-connect useEffect runs
    } catch (e) {
