...
/**
 * SOURCE OF TRUTH — web-public/components/LandingPageContent.jsx is a
 * DUPLICATE of this file (web-public's Turbopack config pins its build
 * root to that project's own directory, so it can't cross-import this one
 * — confirmed empirically, see that file's own header comment for the
 * full reasoning). Keep both in sync on every edit, same as this repo's
 * ideaStatusSummary() (src/utils/format.js / web-public/lib/format.js) and
 * SOCIAL_LINKS (src/constants/app.js / web-public/app/page.jsx).
 *
 * The actual landing-page content — copy, layout, CSS — is shared between
 * two completely different runtimes:
 *   - src/features/marketing/LandingPage.jsx (this repo's Vite/React SPA,
 *     rendered client-side, "Sign in"/"Create account" switch to LoginPage
 *     via local state, no page reload)
 *   - web-public/app/page.jsx (a separate Next.js project, server-rendered,
 *     no Firebase/auth of any kind — see that project's own README for why)
 *
