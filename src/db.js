...
// ─────────────────────────────────────────────────────────────────────────────
// Phase 3: API-first, fallback-to-direct-Neon helper (mirrors the pattern
// established in src/AuthContext.jsx for Phase 2). Every exported function
// below tries the authenticated server endpoint first; only a genuine
// infrastructure problem (network error, 5xx, unreachable) falls through to
// the original direct-Neon query kept beneath it. An explicit 401/403 from
// the API is an authorization decision, not an infra failure — it is NEVER
// treated as a reason to fall back to the privileged direct-Neon path.
// ─────────────────────────────────────────────────────────────────────────────
const API_BASE = (import.meta.env.VITE_CAS_API_URL || "https://investor-circle.vercel.app") + "/api";
...
export async function getPublicProfile(username) {
  ...
  // Phase 3: try the (unauthenticated-by-design) server-side endpoint first —
  // it runs against DATABASE_URL rather than the browser-exposed
