...
/**
 * DUPLICATE — the source of truth is
 * src/features/marketing/LandingPageContent.jsx in the main repo (the
 * Vite/React SPA project). Keep the two in sync on every edit, same as this
 * project's own ideaStatusSummary() in lib/format.js and the SOCIAL_LINKS
 * list in app/page.jsx.
 *
 * Why a duplicate, not a cross-import: this project's next.config.mjs pins
 * Turbopack's `root` to this directory specifically (see that file's own
 * comment — without it, Turbopack picks up the main app's lockfile and
 * warns on every build). That root scoping also means Turbopack refuses to
 * resolve an import reaching outside this directory — confirmed empirically
 * (a `../../src/features/marketing/LandingPageContent.jsx` import fails
 * the build with "Module not found") — so a cross-repo import isn't
 * actually available here without loosening that config, which isn't worth
 * doing for one shared file. This mirrors the same trade-off this project
 * already made for ideaStatusSummary()/SOCIAL_LINKS.
 *
 * The actual landing-page content — copy, layout, CSS — is shared between
 * two completely different runtimes:
 *   - src/features/marketing/LandingPage.jsx (the SPA, rendered
 *     client-side, "Sign in"/"Create account" switch to LoginPage via
 *     local state, no page reload)
 *   - web-public/app/page.jsx (this project, server-rendered, no
 *     Firebase/auth of any kind — see this project's own README for why)
 *
 * Deliberately zero-dependency: no Vite (`import.meta.env`), no
 * react-router, no Firebase, no hooks beyond what plain React offers — kept
 * that way so copying this file over during a sync is a clean, mechanical
 * diff, not a rewrite.
 *
 * The two pieces of behavior that legitimately differ between the two
 * callers — what "Sign in" and "Create account" actually DO — are the only
 * things passed in, as render-prop functions rather than component types
 * (so a fresh element identity each render doesn't force a remount):
 *   signInCTA(props)        -> ReactNode
 *   createAccountCTA(props) -> ReactNode
 * The SPA passes functions that render a <button onClick=...>; web-public
 * passes functions that render a real <a href="https://myinvestorcircle.com/?next=/...">
 * (see that file's own comment for why that particular URL makes the SPA's
 * existing ?next= handling take over with zero App.jsx changes).
 *
 * `socialLinks` is passed in rather than imported for the same reason —
 * the two projects each already hold their own copy of this small, rarely
 * -changing constant (src/constants/app.js's SOCIAL_LINKS here, and
 * web-public would reuse the same list) rather than adding a second
 * cross-import for one array.
 *
 * The three in-page nav links ("How it works" / "Your circle" / "Questions")
 * are real `<a href="#lp-how">` anchor links with CSS `scroll-behavior:
 * smooth`, not onClick + scrollIntoView — this works identically in both
 * runtimes with zero JavaScript, which is what lets web-public ship this
 * page with no required client-side hydration for it at all.
 */
...
/* Scoped to the lifetime of this component's own inline stylesheet
   (removed from the DOM the moment it unmounts), so this bare html{} rule
   never leaks into either host app's other pages. Needed because the
   in-page nav links
   below are now real #anchor links, not onClick+scrollIntoView — this is
   what makes that jump animated instead of an instant snap, with no JS. */
    ...
    <div className="lp">
      ...
      <div className="lp-nav">
        <div className="lp-wrap lp-navrow">
          ...
          <div className="lp-navlinks">
            <a className="lp-navlink" href="#lp-how">How it works</a>
            <a className="lp-navlink" href="#lp-circle">Your circle</a>
            <a className="lp-navlink" href="#lp-faq">Questions</a>
          </div>
          ...
          <div className="lp-card" style={{borderRadius:18}}>
            ...
            <div style={{padding:'16px 24px 24px',display:'flex',flexDirection:'column',gap:10}}>
              {signInCTA({
                ...
                style: {width:'100%',padding:'12px',borderWidth:'1.5px',borderColor:'#e8e8f2',fontSize:14},
                children: (
                  <>
              ...
              <div style={{marginTop:10,paddingTop:16,borderTop:'1px solid #e9e9f2',textAlign:'center',fontSize:13,color:'#565a78'}}>
                ...
                {createAccountCTA({
                  style: {background:'none',border:'none',padding:0,cursor:'pointer',
                    fontFamily:'inherit',fontSize:13,fontWeight:700,color:'#5a49e6',textDecoration:'none'},
      ...
      <div className="lp-wrap lp-sec" style={{textAlign:'center'}}>
        ...
        <div style={{display:'flex',gap:12,justifyContent:'center',marginTop:28,flexWrap:'wrap'}}>
          {createAccountCTA({ className: "lp-btn lp-btn-pri lp-btn-lg", children: "Create your account" })}
          {signInCTA({ className: "lp-btn lp-btn-ghost lp-btn-lg", children: "Sign in" })}
        </div>
      </div>
      ...
      <div style={{background:'#fff',borderTop:'1px solid #e9e9f2'}}>
        <div className="lp-wrap" style={{paddingTop:40,paddingBottom:40,display:'flex',flexDirection:'column',alignItems:'center',gap:18}}>
          ...
          {/* Real navigation, not an onClick — the one place on this page
              that links to actual public content (/search, which fans out
              to public Stock Insights pages) rather than scrolling within
              the page or triggering the sign-in/create-account flow. */}
